 ...
 */
class TableComments extends TableCommentsEvents
{
    /**
     * @param int $tableId
     * @param int $page
    ...
    public static function getComments(int $tableId, int $parentId = -1, $page = 0, $limit = Paging::endlessScrollPortions)
    {
        $query = self::query()
                     ...
                     ->orderBy(self::class . '.votesCount DESC');
    ...
    public static function getChildComments(int $tableId, int $parentId = -1, $page = 0, $limit = Paging::endlessScrollPortions)
    {
        $query = self::query()
            ->columns(
                [
            ...
            ->where(self::class . '.tableId = :tableId:', ['tableId' => $tableId])
            ->limit((int)$limit, $limit * $page)
            ->orderBy(self::class . '.createdAt DESC');
