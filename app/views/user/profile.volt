{% extends 'layouts/main.volt' %} {% block title %}SpreadShare - {{ profile.name }}{% endblock %} {% block header %} {% endblock
%} {% block content %}
...
<div class="re-page">
    <div class="profile-info u-flex u-sm-flexCol u-md-flexRow u-flexJustifyBetween u-md-flexAlignItemsStart">
        ...
        <div class="profile-info__col2">
            <div class="profile-image" id="changeProfileImage" style="background: url('{{ profile.image }}') center / cover;">
    ...
    <div class="u-flex u-flexWrap med-gutter">
        ...
        <div class="list-tab-content-subscribers__card">
            {{ partial('partials/profile-card', [ 'id': user['id'], 'username': user['handle'], 'avatar': user['image'], 'name': user['name'],
            'bio': user['tagline'], 'type': 10, 'truncate': true ]) }}
        </div>
        {% endfor %}
    </div>
    {% endif %} {% if currentPage == "upvoted" %}
    <div class="u-flex u-flexWrap big-gutter">

        {% if createdLists is empty %} {% if auth.loggedIn() and auth.getUserId() == profile.id %}
        <div class="info-box">
            <div class="info">
        ...
        <div class="info-box">
            <div class="info">
                <div class="heading"> No Streams </div>
    ...
    $(document).ready(function () {
        ...
        document.querySelector('input[type="file"]').addEventListener('change', function () {
            if (this.files && this.files[0]) {
                ...
                img.attr('style', 'background: url(' + URL.createObjectURL(this.files[0]) +
                    ') center / cover;');
                //img.onload = fn;
            }
        });
