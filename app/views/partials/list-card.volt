...
<a href="/stream/{{ id }}" class="list-card__subscriberCount u-flex u-flexAlignItemsStart">
  <img src="/assets/images/9-0/list-card-subscriber-bird.svg" /> {{ subscriberCount ? subscriberCount : 0 }}</a>
  </div>
  <p class="list-card__description">{% if tagline is defined %}{{ tagline }}{% else %}{{ description }}{% endif %}{% if showCurator %}, curated by <a href="/profile/{{ curatorHandle }}">{{ curatorName }}</a>{% endif %}</p>
  {# {% if showCurator %}
