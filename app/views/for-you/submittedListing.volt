            ...
            <tr data-id="{{ listing.id }}" class="list-row-tr">
                ...
                <td style="text-align:left;width:85%;">
                    ...
                    {% if listing.columns[2].link is defined AND (listing.columns[2].link OR listing.columns[2].content) %}
                        <a class="re-table-link" target="_blank" href="{{ listing.columns[2].link }}">{{ truncate(listing.columns[2].content ? listing.columns[2].content : listing.columns[2].link, 100) }}</a>
                    {% endif %}
