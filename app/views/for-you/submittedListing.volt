...
<div class="table-scroll table-scroll--collaborations">
 ...
 SPREADS
                ...
                <th>{# image #}</th>
                {% set columns = listing.columns %} {% for column in columns %}
                <th>{{ column.name }}</th>
                {% endfor %}
            </tr>
        </thead>
...
<img class="vote-link__image vote-link__image--green" src="/assets/images/vote-lightning-green.svg" />
                    ...
                    {% for column in columns %} {% set len = filterTableRowsContent(column.content)|striptags|length %} {% if len > 160 %} {%
                    set length = 600 %} {% elseif len > 80 %} {% set length = 300 %} {% elseif len > 40 %} {% set length
                    = 175 %} {% elseif len > 20 %} {% set length = 150 %} {% else %} {% set length = 0 %} {% endif %}
                    <td style="min-width: {{ length }}px;">{{ filterTableRowsContent(column.content) }}</td>
                    {% endfor %}
                </tr>
