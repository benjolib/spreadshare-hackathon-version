...
{% if !amISubscribed(table['id']) %}
            <div class="list-actions u-sm-flexCol u-md-flexRow">
                ...
                <div class="sh-dropdown card-subscribe-dropdown card-actions-dropdown--tall u-flex u-flexCol l-dropdown" style="padding-left: -100px;display:none;visibility: hidden">
            ...
            <div class="list-actions u-sm-flexCol u-md-flexRow u-sm-flexAlignItemsCenter">
                <div class="button-container button-container__white" data-dropdown-placement="center" data-dropdown-offset="0">
                    ...
                    <div class="info">
                        ...
                        <u><b><a href="/subscriptions">here</a></b></u>
                    </div>
                </div>
                <div class="sh-dropdown card-subscribe-dropdown card-actions-dropdown--tall u-flex u-flexCol l-dropdown" style="padding-left: -100px;display:none;visibility: hidden">
                    <a href="javascript:;" onclick="subsFreqOnClick({{ table['id'] }}, 'D')">Daily</a>
...
{% if tableContent is not empty and tableContent %}
    {% for index, row in tableContent.items %}
                <tr data-id="{{ row['id'] }}" class="list-row-tr">
                    ...
                    <td class="shadowcontaintd">
                        <div class="shadowcontain dropright">
                            <div
                                class="l-button"
                                style="position: absolute;top: 0px;right: 6px;pointer-events: all;cursor: pointer;"
                                data-dropdown-placement="top-start"
                                data-dropdown-offset="0"
        ...
        {% for i, cell in row['content']|json_decode %}
                ...
                <tr class="list-row-tr">
                    <td class="pagination-td">
                        <div class="pagination">
                            <a href="?page=1&orderby={{orderby}}"><<</a>
                            <a href="?page={{ tableContent.before }}&orderby={{orderby}}"><</a>
...
{% for index,column in tableColumns %}
                    <td>
                        <div style="display:flex;">
                            <textarea
                                style="min-width: {{ column.title|length*16 }}px !important;"
                                onmouseover="javascript:$('.e{{i}}{{index}}').css('visibility','visible');" ;
                                onmouseout="javascript:$('.e{{i}}{{index}}').css('visibility', 'hidden');"
                                id="{{i}}"
                                placeholder="{{ column.title|ucfirst }}"
                                rows="1"
                                class="edit icon cell-input-sizing d{{i}}{{index}}"
                            >
                            </textarea>
...
{% endfor %}
        ...
        <div class="addAListingSubmitAndCancel" id="addAListingSubmitAndCancel" style="display: none;">
            <a class="re-button re-button--list-add-row" href="#" id="addAListingSubmit">
                <img src="/assets/images/9-0/list-collaborate-button-octopus.svg" />
                Submit
            </a>
            <a style="width:140px" class="re-button re-button--list-add-row-cancel re-button--grey small-cancel-button" href="#" id="addAListingCancel">
                Cancel
            </a>
        </div>
...
{% if !amISubscribed(table['id']) %}
        <div class="list-actions bottom u-sm-flexCol u-md-flexRow u-md-flexRow">
            <div class="button-container button-container__orange l-button" data-dropdown-placement="center" data-dropdown-offset="0">
                <a class="re-button re-button--list-subscribe" href="javascript:;" onclick="subsFreqOnClick({{ table['id'] }}, 'W')">
                    <img src="/assets/images/9-0/bird-orange.svg" />
                    Subscribe
                </a>
            ...
            <div class="button-container button-container__blue">
                <div class="but">
                    <a class="re-button re-button--list-collaborate addAListingButton1" href=" # ">
                        <img src="/assets/images/9-0/octopus-blue.svg" />
                        Collaborate
                    </a>
                </div>
                <div class="info">
            ...
            <div class="about-list__item">
                ...
                <div class="about-list__item__content">
                    <div class="about-list__part">
                        <b>{{ table['subscriberCount'] }}</b>
                        Subscriptions
                    </div>
                    <div class="about-list__part">
                        <b>{{ table['contributionCount'] }}</b>
                        Collaborations
                    </div>
                    <div class="about-list__part">
                        <b>{{ table['commentsCount'] }}</b>
                        Comments
                    </div>
                </div>
            </div>
...
{{ partial('partials/profile-card', [
    ...
    'commentId': comment['id'] ]) }}
                <div class="comment-clock">
                    <img src="/assets/images/comment-clock.svg" />
...
{{ partial('partials/profile-card', [
    ...
    'commentId': comment['id'] ]) }}
                <div class="comment-clock">
                    <img src="/assets/images/comment-clock.svg" />
                    {{ formatTimestamp(childComment['createdAt']) }}
                </div>
            </div>
        {% endfor %}
        {% if auth.loggedIn() %}
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        function unbindListCellEditableSizing() {console.log('bloff');
                    ...
                    text = text.replace(
                        ...
                        function (match) {
                            return '<a class="re-table-link" target="_blank" title="' + match + '" href="' +
                                match + '">' + (new URL(match))['host'].replace('www.', '') + '</a>';
                        }
                    );
                    $this.html(text);
                });
    ...
    $('#share-hacker').on('click', function (e) {
        e.preventDefault();
        var txt = 'Check this spreadsheet out! ';
        window.open('https://news.ycombinator.com/submitlink?u=' + encodeURIComponent(url) + '&t=' + encodeURIComponent(title),
            '', 'left=0,top=0,width=550,height=450,personalbar=0,toolbar=0,scrollbars=0,resizable=0');
    });
